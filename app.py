...
def financeiro_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        return f(*args, **kwargs)
    return decorated
def membro_required(f):  # NOVO: Decorador para rotas só de membros (nível > 3)
    @wraps(f)
    def decorated(*args, **kwargs):
        if not current_user.is_authenticated or not current_user.membro_id or current_user.nivel_acesso <= 3:
            flash("Acesso negado. Apenas membros comuns.", "danger")
...
class UsuarioForm(FlaskForm):
    ...
    nivel_acesso = SelectField("Nível de Acesso", choices=[
        ...
        (5, "Membro (apenas financeiro pessoal)")  # Ajustado para clareza
...
def membros_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if current_user.nivel_acesso not in [1, 2, 3]:
            flash("Acesso permitido apenas à Secretaria.", "danger")
            return redirect(url_for('secretaria'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.senha, form.senha.data):
            ...
            if user.membro_id and user.nivel_acesso > 3:  # Membro comum: redireciona para financeiro pessoal
                return redirect(url_for("financeiro_membro"))
            elif user.nivel_acesso <= 3:  # Admin/Secretária: dashboard secretaria
                return redirect(url_for("secretaria"))
            else:
...
@login_required
@financeiro_required  # Agora usa financeiro_required, que bloqueia membros comuns (>3)
def financeiro():
    form = TransacaoForm()
    membros = Membro.query.order_by(Membro.nome).all()
...
@login_required
@membro_required  # NOVO: Protege para apenas membros comuns
...
# ================================
genai.configure(api_key="AIzaSyC9Be9F-sWq25MfVEbC3Hf_Ute-18lgPUU")
